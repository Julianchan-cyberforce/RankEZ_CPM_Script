...
def handle_request(request):
    ...
    try:
        x_api_id = request["pluginData"]['properties']["x_api_id"]
        x_api_key = request["pluginData"]['properties']["x_api_key"]
    ...
    try:
        ...
        results = requests.get(url, headers={"x-API-Id": x_api_id, "x-API-Key": x_api_key})
        ...
        for result in response_data["data"]:
            ...
            lastLogonTime = datetime.strptime(result["lastLoginTime"][:-6], "%Y-%m-%dT%H:%M:%S.%f").timestamp()
            createdTime = datetime.strptime(result["created"][:-6], "%Y-%m-%dT%H:%M:%S.%f").timestamp() 
