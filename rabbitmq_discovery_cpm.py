...
def handle_request(request):
    ...
    # Construct the URL
    url = f"{request['address']}/api/users"

    # Make the API request
    results = requests.get(url, auth=HTTPBasicAuth(request['account'], request['password']))
    ...
    if results.status_code >= 400:
      ...
      response['errorMsg'] = f"{request['address']}/api/users error: {results.status_code} - {response_data['reason']}"
      return response
