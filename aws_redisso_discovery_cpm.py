...
def handle_request(request):
    ...
    try:
        ACCESS_KEY = request["pluginData"]['properties']["access_key"]
        SECRET_ACCESS_KEY = request["pluginData"]['properties']["secret_access_key"]
        REGION_NAME = request["pluginData"]['properties']["region_name"]
        
        client = boto3.client('elasticache', aws_access_key_id=ACCESS_KEY, aws_secret_access_key=SECRET_ACCESS_KEY, region_name=REGION_NAME)
        
        results = client.describe_users()
        print(results)
        ...
        for result in results:
            ...
            if "administrator" in result['tags']:
                scan_result.update({
                    "privileged": true,
                }) 
        ...
        response["code"] = 1
        response["errorMsg"] = f"Coding error: {str(e)}"
